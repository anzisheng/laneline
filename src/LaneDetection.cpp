...
#define VALID_SEED_MARKING_NUMBER_THRES 10
...
bool LaneDetection::initialize_variable(std::string& img_name) {
	...
	img_roi_height = (int)(img_size.height*0.1);
...
bool LaneDetection::initialize_Img(std::string& img_name) {
	...
	cv::imshow("origin", freespace_img_out);

	img_gray = cv::Mat(img_size, img_depth);
	if (freespace_img_out.channels() == 1) {
		freespace_img_out.copyTo(img_gray);
	}
	else {
		cv::cvtColor(freespace_img_out, img_gray, CV_BGR2GRAY);
	}
